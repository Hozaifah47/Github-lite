  ...
  repos.forEach(r => {
    ...
    div.innerHTML = `
      <div style="display:flex;justify-content:space-between">
        ...
        <div><button class="btn" data-id="${r.id}">Open</button></div>
      </div>`;
  ...
  repoViewEl.innerHTML = `
    <div class="header-row">
      <div>
        ...
        <div class="small">${currentRepo.description || ''}</div>
      </div>
      <div>
  ...
  files.forEach(f => {
    ...
    fdiv.innerHTML = `
      <div style="display:flex;justify-content:space-between">
        <div>${f.path}</div>
        <div>
          <button class="btn small" data-id="${f.id}" data-action="edit">Edit</button>
          <button class="btn small" data-id="${f.id}" data-action="del">Delete</button>
        </div>
      </div>`;
...
function showAddFile() {
  ...
  document.getElementById('saveFile').addEventListener('click', async () => {
    ...
    await fetch(`/api/repos/${currentRepo.id}/files`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: pathv, content, author: currentUser ? currentUser.name : 'anon' })
    });
    currentRepo = await api(`/repos/${currentRepo.id}`);
...
async function editFile(fileId) {
  ...
  document.getElementById('saveEdit').addEventListener('click', async () => {
    ...
    await fetch(`/api/repos/${currentRepo.id}/files/${fileId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: newPath, content: newContent, author: currentUser ? currentUser.name : 'anon', message: `Edited ${newPath}` })
    });
    currentRepo = await api(`/repos/${currentRepo.id}`);
    closeModal(); renderRepoView();
  });
...
async function showCommits() {
  const commits = await api(`/${currentRepo.id}/commits`) || [];
  let html = `<h3>Commits</h3><div style="max-height:300px;overflow:auto">`;
  commits.slice().reverse().forEach(c => {
    html += `<div class="commit-item">
      <b>${c.message}</b>
      <div class="small">by ${c.author||'anon'} at ${c.time}</div>
      <div style="margin-top:6px"><button data-cid="${c.id}" class="btn revert">Revert to this</button></div>
    </div>`;
  });
  html += `</div>`;
  showModal(html);
  ...
  modal.querySelectorAll('.revert').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      await fetch(`/api/repos/${currentRepo.id}/revert`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ commitId: cid, author: currentUser ? currentUser.name : 'anon' })
      });
      currentRepo = await api(`/repos/${currentRepo.id}`);
      closeModal(); renderRepoView();
    });
